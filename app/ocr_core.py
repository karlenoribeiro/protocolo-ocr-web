...
OCR_LANGS = _langs()
# ... (Funções to_gray, rotate_bound, deskew_osd, enhance_for_ocr, tesseract_config, ocr_text, ocr_data, find_protocolo_roi, extrair_protocolo_de_texto - Mantidas) ...
def to_gray(img):
    if len(img.shape) == 3:
...
# --- NOVAS FUNÇÕES DE LEITURA E ESCRITA (ROBUSTEZ/DUPLICIDADE) ---
...
def ler_protocolos_existentes() -> Tuple[set, pd.DataFrame]:
    ...
    df_existente = pd.DataFrame(columns=["arquivo","protocolo_ocr","protocolo_17_digitos","data_extracao", "ok"])
    ...
    # Retorna o set de protocolos válidos e o DataFrame original lido (pode conter colunas indesejadas que serão limpas no consolidar)
    return validos, df_existente

def salvar_excel_texto(df_total: pd.DataFrame):
    """Salva DataFrame em arquivo Excel com escrita atômica (temporário + renomear) e formatação de texto."""
    ...
    # Gravação Atômica: Grava em arquivo temporário e depois renomeia
    ...
    try:
        with pd.ExcelWriter(temp_path, engine="openpyxl", mode="w") as writer:
            ...
            # Formatação de coluna como texto
            try:
...
def consolidar_resultados(novos: List[dict], df_existente: pd.DataFrame) -> pd.DataFrame:
    ...
    # Cria DataFrame com os novos resultados VÁLIDOS (já filtrados no main_web.py)
    df_novos = pd.DataFrame(novos)
    
    # Garante colunas mínimas para evitar erro de concatenação
    colunas_padrao = ["arquivo","protocolo_ocr","protocolo_17_digitos","data_extracao", "ok"]
    for col in colunas_padrao:
        if col not in df_novos.columns:
            df_novos[col] = None
    
    # Junta dados novos e antigos
    # O df_existente pode ter sido lido com colunas extras, mas a concatenação deve funcionar
    df_total = pd.concat([df_existente.drop(columns=[col for col in df_existente.columns if col not in colunas_padrao], errors='ignore'), df_novos], ignore_index=True)
    ...
    df_total = df_total[df_total['protocolo_17_digitos'].str.len() == 17].copy() # Usa .copy() para evitar SettingWithCopyWarning
    
    # 3. CRUCIAL: Remove duplicatas no arquivo final APENAS pelo protocolo de 17 dígitos
    # Isso garante que um protocolo só exista uma única vez no Excel, independente do nome do arquivo.
